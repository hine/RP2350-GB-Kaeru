...
| 2026-06-05 | CH4 への LPF 適用は不採用 | 25% スケールで十分な音質が得られたと判断。混合後バッファへの LPF 案（α=11/16）も実施せず |
| 2026-06-05 | FatFS LFN を malloc → 静的バッファに変更（`FF_USE_LFN 3` → `1`） | ゲーム起動後はヒープが枯渇しており、SD バックアップ時に Out of Memory が発生していたため |
| 2026-06-05 | Backup to SD / Reset に確認ダイアログを追加（AMOLED・PicoCalc 両対応） | 誤タップ・誤操作によるデータ上書きを防ぐため |
| 2026-06-05 | AMOLED ステータスバーの SAVE / LOAD に確認ダイアログを追加（スロット番号表示） | 誤タップ防止。`do_confirm_open/close()` を新設し、設定保存なし・ステータス維持で閉じる |
| 2026-06-05 | セーブステートの有効判定をフラグ管理に移行（`states_meta_t` を metadata offset 40 に追加） | Clear Flash のメタデータ消去だけで全スロットを無効化できるようにするため。320KB の物理消去が不要になり ~50ms で完了 |
| 2026-06-05 | Clear Flash ダイアログ説明文を実態に合わせて修正（両ターゲット） | "ROM/SRAM/Settings will be erased" は不正確。実際はフラグと設定のみクリアし物理データは残る |
| 2026-06-05 | README.md と flash_meta.h の Flash レイアウトを修正（10スロット・0x1E0000） | コメントが 11スロット・0x1C8000 と誤記されていた |
| 2026-06-05 | README.md に picotool による完全消去手順を追記 | Clear Flash はフラグのみ操作。物理削除が必要な場合の手順をドキュメント化 |
| 2026-06-05 | Full Erase Flash メニュー項目を両ターゲットに実装 | ROM / SRAM セーブ / セーブステート全スロット / メタデータを物理消去（約2.5秒）。完了後 watchdog リブート。AMOLED は「Erasing Flash...」表示後にロックアウト → 消去 → リブートの順。Clear Flash（~50ms）との使い分けを README に表で整理 |
...
| 2026-06-05 | AMOLED メニュー UI 実装完了 | タッチメニュー（8項目: パレット / 音量 / Backup to SD / Restore from SD / Clear Flash / Full Erase Flash / Reset / Close）。確認ダイアログ + トースト方式 |
| 2026-06-05 | AMOLED USB stdio を無効化・デバッグ printf を全削除 | リリース向けクリーンアップ。AMOLED は元々 stdio_usb=1 で USB シリアルに出力していた |
| 2026-06-05 | SD 操作時に「No SD card」エラートーストを追加（両ターゲット） | SDなしで Backup/Restore を実行すると "No save data" / "Load failed" という誤解を招くメッセージが出ていたため。マウント失敗を明示的に判定するよう変更 |
