...
調査結果の詳細は `buttons.md` を参照。

| ボタン | 回路上の名称 | 接続先 | ファームウェアでの扱い |
|-------|------------|--------|----------------------|
| POWER / PWR | Key1 | AXP2101 PWRON + **GPIO18** | **A ボタン（GPIO18 で読み取り）** |
| BOOT | Key2 | QSPI_SS_N | 書き込みモード専用（通常 GPIO として使用不可） |
...
**USB 給電中は安全**：AXP2101 が VBUS を検出して電源 OFF 処理を無効化するため、長押しでも GPIO18 入力として安全に読める。

**バッテリー使用時は要注意**：長押し（約 4 秒）で AXP2101 が電源 OFF を実行する可能性がある。実装時は押下時間を計測し、閾値（例: 2 秒）超えで A 入力を中断するロジックが必要。
